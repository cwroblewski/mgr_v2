...
class Weapon(models.Model):
    name = models.CharField(
        verbose_name="nazwa", max_length=1024, unique=True, db_column="nazwa"
    )
...
class Factor(models.Model):
    ...
    parameter = models.CharField(
        verbose_name="parametr", max_length=64, db_column="parametr"
    )
    temperature = models.SmallIntegerField(
        verbose_name="temperatura", db_column="temperatura"
    )
...
class Material(models.Model):
    material_type = models.CharField(
        verbose_name="materiał", max_length=1024, blank=True, db_column="material"
    )
...
class Base(models.Model):
    base = models.CharField(
        verbose_name="podłoże", max_length=512, unique=True, db_column="podloze"
    )
...
class Shot(models.Model):
    ...
    bullet = models.ForeignKey(
        Bullet, on_delete=models.DO_NOTHING, db_column="id_pocisku"
    )
    temperature = models.FloatField(blank=True, null=True, db_column="temperatura")
    atmosferic_conditions = models.TextField(
        verbose_name="warunki_atmosferyczne", db_column="warunki_atmosferyczne"
    )
    wind_speed = models.IntegerField(
        verbose_name="predkosc_wiatru", db_column="predkosc_wiatru"
    )
    material = models.ForeignKey(Material, models.DO_NOTHING)
    base = models.ForeignKey(Base, models.DO_NOTHING, db_column="podloze")
    ricochet = models.ForeignKey(
        Ricochet, models.DO_NOTHING, db_column="rykoszet", null=True, blank=True
    )
    factor = models.ForeignKey(Factor, models.DO_NOTHING, db_column="czynnik")
    link = models.CharField(max_length=80)
    link_caliber = models.CharField(
        verbose_name="link kaliber",
        max_length=80,
        default="1",
        db_column="link_kaliber",
    )
    camera = models.CharField(
        verbose_name="kamera zwykła",
        max_length=64,
        default="1",
        db_column="kamera_zwykla",
    )
    slowmotion_camera = models.CharField(
        verbose_name="kamera szybka",
        max_length=64,
        default="1",
        db_column="kamera_szybka",
    )
    ir_camera = models.CharField(
        verbose_name="kamera ir", max_length=64, default="1", db_column="kamera_ir"
    )
    photo = models.ImageField(
        verbose_name="zdjęcie", upload_to="images/", null=True, db_column="zdjecie"
    )
